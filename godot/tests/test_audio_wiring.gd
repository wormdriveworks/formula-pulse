...
const PENDING := {
	"result_neutral": "무사건 확정 턴이 엔진에 없다 — 정산 ②가 트러블 부재 시 반드시 안정을"
		+ " 발행한다(섹터 턴 480건 전수 실측 0). D05·D11 정합 판정 대기",
	"sell": "튜닝 재배분(환급) 경로 미결선 — HUB-03 골격이 구매만 연다",
	"crew_join": "크루 영입 스테이션(StRecruit) 미구현 — 라우트 자체가 빈 칸이다",
	"skill_hold": "스킬 발동 미결선 — RACE-01 덱 슬롯 전량 잠금 표기",
	"skill_convert": "스킬 발동 미결선 (동일)",
	"skill_amplify": "스킬 발동 미결선 (동일)",
	"skill_insure": "스킬 발동 미결선 (동일)",
