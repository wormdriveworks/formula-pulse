...
# 개러지(HUB-01)에서 마지막으로 진입한 스테이션의 라우트 — 하위 화면에서 돌아온 개러지가 초기 포커스를
# 그 앵커에 둔다(개선 회차 16 · 2026-09-11 사용자 요청: 복귀마다 첫 스테이션으로 튀던 것). 라우터가 화면을
# 매번 새로 세우므로 화면 인스턴스는 직전 위치를 모르고, 세션이 쥔다. 입력 편의 래치라 세이브 대상이 아니며
# 출발(`_on_depart`)이 비운다 — 레이스에서 돌아온 개러지는 종전대로 첫 스테이션(H6 기본값)이다.
var last_hub_station := ""
