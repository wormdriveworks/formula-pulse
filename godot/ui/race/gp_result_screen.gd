...
# **투어 중 이탈 = 개러지(HUB-01)** (개선 회차 10 · 2026-09-08 사용자 결정 — 간이 정산 화면 소거 ·
# 레이스 ↔ 개러지 반복). 이벤트 노드(RUN-02)는 그 사이에 발생 시에만 선다.
#
...
func _on_next(to_tour_report: bool) -> void:
	if to_tour_report:
		...
		go("SET-01", {})
		return
	# 이벤트 노드 삽입 지점 (D09 §2.3 — RACE-03 → 개러지 사이, 발생 시에만).
	...
	if not occurrence.is_empty():
		...
		return
	# 투어 중에도 개러지로 돌아간다 — GP 사이의 정규 경로는 개러지 → 출발이다 (개선 회차 10).
	go("HUB-01", {})
