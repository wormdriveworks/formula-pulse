...
- **Godot 버전은 4.7 계열 고정** (D12 §1.1 확정 핀 — v1.1 결정 #11·현행 v1.2 — 정본 정합). 다르면 `.tscn`/기능 차이로 디프가 오염된다. 설치 실물 = 4.7.1-stable.
...
- **단일 진실 원천:** `docs/master_plan_v2_59.md`
...
2. **데이터 드리븐 (D12 §4)** — 콘텐츠·수치는 전량 데이터 정의. 코드는 타입·규칙만 보유한다. **수치를 코드에 임의 기입하지 않는다. 필요한 값이 D13에 없으면 작업을 중단하고 보고한다** (값의 유일 창구 = D13 확정 기준값 대장 **95항** — v1.9 기준·채택 94/조정 1). **단 색값은 대장 밖이다** — 팔레트 실색상은 에셋 실물 소관(D13 별첨A 명시 위임·총괄 판정 2026-08-16).
