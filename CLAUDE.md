...
- **단일 진실 원천:** `docs/master_plan_v2_38.md`
...
2. **데이터 드리븐 (D12 §4)** — 콘텐츠·수치는 전량 데이터 정의. 코드는 타입·규칙만 보유한다. **수치를 코드에 임의 기입하지 않는다. 필요한 값이 D13에 없으면 작업을 중단하고 보고한다** (값의 유일 창구 = D13 확정 기준값 대장 **95항** — v1.5 기준·채택 94/조정 1). **단 색값은 대장 밖이다** — 팔레트 실색상은 에셋 실물 소관(D13 별첨A 명시 위임·총괄 판정 2026-08-16).
