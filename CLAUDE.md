...
7. **검증기 = 빌드 게이트 (D12 §4.2 · D14 §2.1 TL-1)** — V1~V8 + 혼입 0 스캔은 빌드 전 단계 고정 실행. **V1~V6·V8 + 혼입 0 스캔 = 위반 1건이면 빌드 실패.** 단 **V7(금칙 어휘 검사)은 경고 전용이며 빌드를 차단하지 않는다** — 작법 판단은 D04 트랙 소관이고 기계는 보조라는 D12 §4.2 V7 행·D14 TL-1의 명문이다. 검증기를 우회·완화·비활성화하지 않으며, **V7을 차단형으로 구현하지도 않는다.** 구현 층 신설 검사 **MIX0·ARCH·FONT·PAL = 차단형**이다 (FONT = 총괄 판정 IMPL-148 — 기계 판정·오검출 0 실측·미적 판단 아님이라 V7과 성격이 다르다 · PAL = 조건부 기승인 IMPL-209 → **위반 0 실측 후 발동** IMPL-219, FONT와 같은 절차). **ANCH(앵커 프리셋 배치)는 경고형 신설**이다 (총괄 판정 IMPL-233 ② — 오검출 표본 수집 후 차단형 전환은 별도 판정. 단 **검사의 실재 자체는 UISCR ⑬축이 차단형으로 받친다** — 경고형 검사는 스스로 죽어도 빌드를 멈추지 못하기 때문이다). 신설 검사의 차단/경고 성격은 구현이 정하지 않고 총괄 판정을 경유한다.
