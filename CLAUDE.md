...
> **⚠ 에이전트 회차 뒤 수동 익스포트가 "권한 오류"로 막히면 `.godot\` 캐시부터 지운다 (2026-08-30 실측 — IMPL-498).** 증상 = 대상 폴더를 바꿔도 실패·새 프로젝트는 성공(원본 쪽 문제). 익스포트는 프로젝트 안쪽(`.godot\exported\` 등)에도 쓰는데, 에이전트 CLI 회차가 만든 캐시의 소유권/ACL이 갈리면 에디터가 그 캐시를 갱신하지 못한다(기제 = ACL 오염 추정·대처 확정). 에디터 종료 → `.godot` 삭제 → 재임포트로 복구. 에이전트 셸을 관리자 권한으로 돌리지 않는 것이 예방.
...
- **단일 진실 원천:** `docs/master_plan_v3_39.md`
