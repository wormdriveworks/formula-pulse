...
- **동일 머신 다중 세션 규약 (2026-08-16 신설 — 실사고 5건 후):** 한 작업 트리에서 여러 세션이 동시에 돌 수 있다(구현 레인 · 에셋 트랙 · **내러티브 레인** · 총괄). 다음을 지킨다.
  ...
  2. **IMPL ID는 `git pull` 직후 최대값+1로 부여**한다. 미리 잡아두면 충돌한다 (실사고 4건 — IMPL-119·121로 재번호).
  3. **공유 파일**(`strings.csv`·`impl_log.md`·`core_params.csv` 등)은 **자기 몫만** 반영한다. 남의 미커밋 변경이 섞였으면 HEAD 복원 후 자기 분만 재적용하고 나머지는 작업 트리에 되돌려 둔다 (IMPL-123 전례).
...
- **단일 진실 원천:** `docs/master_plan_v2_63.md`
...
2. **데이터 드리븐 (D12 §4)** — 콘텐츠·수치는 전량 데이터 정의. 코드는 타입·규칙만 보유한다. **수치를 코드에 임의 기입하지 않는다. 필요한 값이 D13에 없으면 작업을 중단하고 보고한다** (값의 유일 창구 = D13 확정 기준값 대장 **95항** — v1.10 기준·채택 94/조정 1). **단 색값은 대장 밖이다** — 팔레트 실색상은 에셋 실물 소관(D13 별첨A 명시 위임·총괄 판정 2026-08-16).
