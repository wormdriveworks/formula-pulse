...
- **동일 머신 다중 세션 규약 (2026-08-16 신설 — 실사고 5건 후):** 한 작업 트리에서 여러 세션이 동시에 돌 수 있다(구현 레인 · 에셋 트랙 · 총괄). 다음을 지킨다.
  1. **소유 경로를 선언하고 그 밖을 건드리지 않는다.** 예: 에셋 트랙 = `docs/assets/` + `godot/assets/` 전속.
...
- **단일 진실 원천:** `docs/master_plan_v2_57.md`
