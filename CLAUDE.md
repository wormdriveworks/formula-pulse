...
- **동일 머신 다중 세션 규약 (2026-08-16 신설 — 실사고 5건 후):** 한 작업 트리에서 여러 세션이 동시에 돌 수 있다(구현 레인 · 에셋 트랙 · **내러티브 레인** · 총괄). 다음을 지킨다.
  ...
  2. **IMPL ID는 `git pull` 직후 최대값+1로 부여**한다. 미리 잡아두면 충돌한다 (실사고 4건 — IMPL-119·121로 재번호). **긴 회차는 푸시 직전에 최대값을 재확인한다** (2026-08-20 보완 — 충돌 7·8호: 규약대로 잡아도 회차가 길면 그 사이에 선점된다. 충돌은 부여 시점이 아니라 푸시 시점에 확정된다).
...
- **단일 진실 원천:** `docs/master_plan_v2_71.md`
