...
- **단일 진실 원천:** `docs/master_plan_v2_47.md`
...
2. **데이터 드리븐 (D12 §4)** — 콘텐츠·수치는 전량 데이터 정의. 코드는 타입·규칙만 보유한다. **수치를 코드에 임의 기입하지 않는다. 필요한 값이 D13에 없으면 작업을 중단하고 보고한다** (값의 유일 창구 = D13 확정 기준값 대장 **95항** — v1.7 기준·채택 94/조정 1). **단 색값은 대장 밖이다** — 팔레트 실색상은 에셋 실물 소관(D13 별첨A 명시 위임·총괄 판정 2026-08-16).
...
7. **검증기 = 빌드 게이트 (D12 §4.2 · D14 §2.1 TL-1)** — V1~V8 + 혼입 0 스캔은 빌드 전 단계 고정 실행. **V1~V6·V8 + 혼입 0 스캔 = 위반 1건이면 빌드 실패.** 단 **V7(금칙 어휘 검사)은 경고 전용이며 빌드를 차단하지 않는다** — 작법 판단은 D04 트랙 소관이고 기계는 보조라는 D12 §4.2 V7 행·D14 TL-1의 명문이다. 검증기를 우회·완화·비활성화하지 않으며, **V7을 차단형으로 구현하지도 않는다.** 구현 층 신설 검사 **MIX0·ARCH·FONT = 차단형**이다 (FONT = 총괄 판정 IMPL-148 — 기계 판정·오검출 0 실측·미적 판단 아님이라 V7과 성격이 다르다). 신설 검사의 차단/경고 성격은 구현이 정하지 않고 총괄 판정을 경유한다.
