...
| 산출 | 크기 |
|---|---|
| `master_60.png` | **288×916** — 마스터 4블록 + 기능 부속 13 |
| `colorblind_alt.png` | 288×930 — 위 + 기능 13(CB 적용) + 교체 4(분할 칩) |
| `master_60_strip.png` | **60×1 순색** — `tools/palette/` 안에 남는다(`res://` 밖). 비아이콘 원도의 양자화 기준 |
...
| 무엇 | 어디서 |
|---|---|
| 마스터 60색 | `godot/assets/palettes/master_60.gpl` ← 총괄이 지정한 입력 |
...
- 마스터 60 · RGB 중복 0 · **구성 16+16+16+12** 가 ID 접두와 맞는가 (D10 v1.2 §2.3)
- **정본 §2 네온 표 16색 ⇔ `.gpl` 네온 블록 16색이 순서까지 일치**하는가 (IMPL-198 신설 — 자색을 두 곳에 적었으므로 대조가 필요하다. **부분 커버리지다** — 뉴트럴 16·팀 16·환경 12 는 여전히 미대조)
- `colorblind_alt.gpl` 의 앞 **60행**이 마스터와 **완전 동일**한가 (D10 §2.4 — 마스터 본체 무접촉)
- 정본 §5 기능 13 · §6 교체 4 의 개수와 짝
- **교차:** 정본 §6 대체색 ⇔ `.gpl` 꼬리 4 · 정본 §6 기본색 ⇔ §5 해당 색
- **스트립 되읽기 검산** — 쓴 파일을 다시 디코드해 고유색이 **정확히 60**이고 `.gpl`과 1:1인지 본다(총괄 부대조건 ② · IMPL-161). 장식이 한 픽셀이라도 섞이면 여기서 죽는다 — 돌연변이 실측 검출
...
2. `master_60.gpl` / `colorblind_alt.gpl` 을 맞춘다. **구분자는 탭.** 마스터 60색만 담고 기능 13색은 넣지 않는다(파일명 계약 `master_60`).
