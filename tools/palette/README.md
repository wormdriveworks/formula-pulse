...
| `master_56_strip.png` | **56×1 순색** — `tools/palette/` 안에 남는다(`res://` 밖). 비아이콘 원도의 양자화 기준 |
...
- **스트립 되읽기 검산** — 쓴 파일을 다시 디코드해 고유색이 **정확히 56**이고 `.gpl`과 1:1인지 본다(총괄 부대조건 ② · IMPL-161). 장식이 한 픽셀이라도 섞이면 여기서 죽는다 — 돌연변이 실측 검출
