 ...
 *        node tools/audio/sfx_gen.js --only=se_t02,amb_01   (부분 주행 — 생성 회차 분할용)
 */
'use strict';
const fs = require('fs');
...
// 선언한 파라미터가 출력에 도달하는가 (2026-08-19 신설).
// sfxr 의 아르페지오·반복·비브라토는 **시점**을 가진다 — 발화 시점이 포락선 길이 밖이면
// 값을 적어도 소리에 없다. 실제로 5건이 이 상태로 생성됐고(같은 파라미터의 두 항목이
// 바이트까지 동일해져서 드러났다) 파일럿 `se_r04` 도 같은 상태였다. **경고형이다** —
// 파일럿 실물은 이미 귀 판정을 통과했고 값을 고치면 바이트가 바뀌어 재청취가 필요하므로,
// 검사가 빌드를 멈추는 것이 아니라 **매 주행에 보이게** 두는 쪽이 맞다.
function deadParams(id, p) {
  ...
  for (const m of out) warn(`${id}: ${m} — 선언이 출력에 도달하지 않는다`);
}
