 ...
 *
 * ── 크기는 우리 취향이 아니라 **Godot Windows 익스포터의 요구 목록**이다(16/32/48/64/128/256).
 *    그래서 선언 파일로 빼지 않고 상수로 둔다 — **선언 파일에 두면 "조정해도 되는 값"으로 읽힌다.**
 *    ⚠ 초판은 이 자리에 **Windows 셸 표시 규약**(16·32·48·256)을 적었고 그것이 4종의 근거였다.
 *    주력 재산출에서 익스포터가 **64·128 부재 경고**를 냈다(IMPL-507) — **셸이 그리는 목록과
 *    빌드가 요구하는 목록은 다른 것**이고, 우리 산출물을 정하는 쪽은 후자다. 종전 4종은
 *    "틀린 값"이 아니라 **다른 질문에 답한 목록**이었다.
 *
...
// **Godot Windows 익스포터가 요구하는 6종이다** (2026-08-30 · IMPL-507 — 주력 재산출 실측:
//   64·128 부재로 경고). 종전 4종은 Windows 셸 표시 크기(16 작은 아이콘 · 32 · 48 보통 · 256 특대)
//   만 보고 정한 것이었고, **표시 규약과 익스포터 요구가 갈린 자리**였다 — 셸이 쓰는 것과
//   빌드가 요구하는 것은 다른 목록이며, 우리 산출물의 목록은 후자가 정한다.
//   우리 취향이 아니므로 선언 파일로 빼지 않고 상수로 둔다(빼면 "조정해도 되는 값"으로 읽힌다).
