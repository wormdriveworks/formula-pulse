...
for (const stage of names) {
  ...
  // ③ 이음비 계량 — 스크럽 뒤에 잰다. 스크럽이 이음 열을 건드리면 그것도 잡혀야 한다.
  //
  //    **게이트는 ③-1 보정 뒤에 있다** (2026-08-24 순서 교정 · IMPL-381). 처음에는 여기서
  //    바로 차단했는데, 그러면 **납품되는 그림이 아니라 보정 전 그림을 재는** 게이트가 된다.
  //    이 세션에서 같은 실수를 세 번 했다(형태 연속성 · 8-이웃 접촉 · 양자화 전 이음) —
  //    **게이트는 무엇이 참인가가 아니라 눈이 무엇을 보는가를 재야 한다.**
  ...
  const seamRatio = () => { let inner = 0; for (let x = 1; x < W0; x++) inner += colDiff(x - 1, x); inner /= (W0 - 1);
    const seam = colDiff(W0 - 1, 0); return { inner, seam, ratio: inner > 0 ? seam / inner : 0 }; };

  // ③-1 [기각된 경로] 끝 대역 톤 보정 — 2026-08-24 · IMPL-381 · **사용자 눈 검수 기각**.
  //
  //    총괄 승인을 받아 구현했고 **계측 기준으로는 성공했다**: 좌우 28px 대역의 각 열 평균을
  //    중앙 대역 평균으로 당겨(열 안의 명암 구조는 보존) 비네트가 아주르 40.0 → 21.3 ·
  //    미라지 52.1 → 26.1 로 경고선 30 아래까지 내려왔다. 이음도 나빠지지 않았다(0.25 → 0.00).
  //
  //    **그런데 눈에는 없던 것이 생겼다 — 화면 중앙에 창백한 기둥이 섰다.** 이유가 구조적이다:
  //    **408 은 204 타일 2장이므로 "끝 대역"은 끝이 아니라 이음의 속이다.** 좌우를 각각 28px
  //    보정하면 이음에서 두 폭이 맞붙어 **56px 균일 대역**이 되고, 그 자리가 화면에서 가장 잘
  //    보이는 자리다. **비네트는 보정이 가장 눈에 띄는 곳에 살고 있었다.**
  //    강도 0.4 로 낮춰도 아주르는 40.0 → 39.9(양자화 스냅으로 사실상 무변화)여서
  //    *"기둥은 조금 남고 비네트도 남는"* 중간이었다 — 기각.
  //
  //    **그래서 기제를 지웠다. 작동하는 코드를 남겨 두면 다음 편집자를 같은 함정으로 부른다.**
  //    (`face_probe.js` 의 자동 검출 제거와 같은 규율이나 이유는 다르다 — 그쪽은 틀린 값을
  //    냈고, 이쪽은 **설계대로 정확히 작동하는데 접근 자체가 타일 구성에 맞지 않는다.**
  //    타일하지 않는 배경이 생기면 이 경로는 다시 유효하다.)
  //
  //    **이 세션에서 같은 종류의 실수가 네 번째다** — 앞의 셋은 게이트였고(형태 연속성 ·
  //    8-이웃 접촉 · 양자화 전 이음) 이번은 **계측기 자체**다. 비네트 지표는 *"끝 대역 평균이
  //    중앙 평균과 얼마나 다른가"* 를 재므로, **대역을 평평하게 눌러 평균만 맞추면 지표는
  //    만족되고 눈에는 판이 생긴다.** 남은 경로 = 아치 형태 직접 손보정(작화 — 별도 판정).
  //
  //    부수 소득: 이음 게이트를 **픽셀 편집 뒤로** 옮겼다(위 ③). 원래는 스크럽 직후에 있어서
  //    후속 편집이 이음을 건드려도 잡지 못하는 자리였다.

  // ③-1b 이음 게이트 — **픽셀 편집이 모두 끝난 뒤의 그림**을 잰다
  const post = seamRatio();
  const need = s.max_seam_ratio === undefined ? 1.0 : s.max_seam_ratio;
  if (post.ratio >= need) fail(`${stage}: 이음비 ${post.ratio.toFixed(2)}배 (내부 ${post.inner.toFixed(2)} · 이음 ${post.seam.toFixed(2)}) — 요구 ${need} 미만`);
